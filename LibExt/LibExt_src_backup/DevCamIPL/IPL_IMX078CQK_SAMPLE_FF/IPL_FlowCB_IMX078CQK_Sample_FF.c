...
#include "sie_lib.h"
//#NT#2014/02/21#C H Lin -begin
#include "IPL_Ctrl_DZoom_Int.h"
//#NT#2014/02/21#C H Lin -end
...
void IPL_FCB_AlgRSC(UINT32 SelIdx, UINT32 Value)
{
    ...
    else if (Value == SEL_RSC_OFF)
    {
        ...
        //#NT#2014/02/21#C H Lin -begin
        //Add "Wait DZoom Idle" command at the end of RSC off.
        //1. Replace "wait IME frame end" by "wait DZoom Idle" command.
        ...
        while(1)
        {
            if((IPL_DZoomIsIdle(IPL_ID_1) == TRUE))
                break;
            Delay_DelayMs(3);
        }
        //#NT#2014/02/21#C H Lin -end
