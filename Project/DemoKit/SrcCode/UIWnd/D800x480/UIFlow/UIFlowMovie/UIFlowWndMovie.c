...
#include "UIFlowWndMovieRes_790S.c"
#else
...
//#NT#2015/01/21#KS Hung -end
    ...
    //Ux_SendEvent(&CustomMovieObjCtrl, NVTEVT_EXE_DUAL_REC, 1, gbMovieDualRecord);
   ...
   if(GxSystem_GetPowerOnSource() == GX_PWRON_SRC_PWR_SW)
   {
    ...
    if (UIStorageCheck(STORAGE_CHECK_ERROR, NULL) == TRUE)
    {
        if (UI_GetData(FL_CardStatus) == CARD_REMOVED)   
         ...
         Ux_OpenWindow(&UIFlowWndWrnMsgCtrl,2,UIFlowWndWrnMsg_StatusTXT_Msg_STRID_CARDERROR, FLOWWRNMSG_TIMER_KEEP);		
    }
		    ...
		    if (Sensor_CheckExtSensor())
		    	{
		    	    if(SysGetFlag(FL_MOVIE_DUAL_REC))
		    	    {
    				   ...
    				   debug_msg("SysGetFlag(FL_PIPVIEW):%d..\r\n",SysGetFlag(FL_PIPVIEW));	
				    PipView_SetStyle(SysGetFlag(FL_PIPVIEW));
#endif
			    }
		    	}
		#endif					 
		    ...
		    if (Sensor_CheckExtSensor())
		    	{
		    	    ...
		    	    {
#if(_MODEL_DSC_ == _MODEL_DUAL_GEMEIDI_) 	   
    				   debug_msg("SysGetFlag(FL_PIPVIEW)-22-:%d..\r\n",SysGetFlag(FL_PIPVIEW));	
    ...
    case MOV_ST_REC|MOV_ST_ZOOM:
       ...
       //if(bCarGuideLineMode==FALSE)
        ...
        else
        {
            UxState_SetData(&UIFlowWndMovie_Status_RECCtrl,STATE_CURITEM,UIFlowWndMovie_Status_REC_ICON_TRANSPAENT);
#if(_MODEL_DSC_ == _MODEL_DUAL_GEMEIDI_) 	   
            LED_TurnOffLED(KEYSCAN_LED_GREEN);
#endif				
        }
       //}
...
INT32 UIFlowWndMovie_OnStorageSlow(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
	...
	debug_msg("---UIFlowWndMovie_OnStorageSlow---\r\n");
	debug_msg("---UIFlowWndMovie_OnStorageSlow---\r\n");
	debug_msg("---UIFlowWndMovie_OnStorageSlow---\r\n");
#if ((_MODEL_DSC_ == _MODEL_CARDV_B50_)||(_MODEL_DSC_== _MODEL_DUAL_NAZHIDA_))
...
inline static void  UIFlowWndMovie_UpdateSpeedData(void)
{
      ...
      static UINT32 DataCountPre=0;	  
      static UINT32 DataCountLost=0;	  
      UINT32 year,month,day,hour,minute,second;	  
      UINT32 DataCountCurrent=0;	  
      UINT32 DataCountTemp=0;	  
      static BOOL gbUpdateUTCTime=FALSE;
	  
      dataValid=GPSRec_GetRMCDate(&RMCInfo);

      memcpy(&(gpsdata.rmcinfo),&RMCInfo,sizeof(RMCINFO));
      if(dataValid==TRUE)
      {
      		if(UxState_GetData(&UIFlowWndMovie_Status_GPSCtrl,STATE_CURITEM)!=UIFlowWndMovie_Status_GPS_ICON_GPS_OK)
      		{
		      ...
		      if(gbUpdateUTCTime==FALSE)	  
		      	{
				gbUpdateUTCTime=TRUE;		      	
				year   = gpsdata.rmcinfo.Year;
				month  = gpsdata.rmcinfo.Month;
				day    = gpsdata.rmcinfo.Day;	
				hour   = gpsdata.rmcinfo.Hour;
				...
				if(hour>23)
				{
					...
					switch(month)
					{
						...
						break;
						case 2:
							if(rtc_isLeapYear(year+2000)==TRUE)
							{
								if(day>29)							
								day=1;	
								month++;
      ...
      {
		DataCountCurrent=GPSRec_GetDataCount();
		gbUpdateUTCTime=FALSE;
		//debug_msg("DataCountCurrent:%d..DataCountPre:%d..\r\n",DataCountCurrent,DataCountPre);
		if(DataCountCurrent!=DataCountPre)
		{
			DataCountTemp=(DataCountCurrent%4+2);
			DataCountLost=0;
    			UxState_SetData(&UIFlowWndMovie_Status_GPSCtrl,STATE_CURITEM,DataCountTemp);
    			UxCtrl_SetShow(&UIFlowWndMovie_Status_GPSCtrl, TRUE);			
		}
		else
		{
			DataCountLost++;
			//debug_msg("DataCountLost:%d..%d\r\n",DataCountLost,UxState_GetData(&UIFlowWndMovie_Status_GPSCtrl,STATE_CURITEM));			
			if(DataCountLost>6)
			{
		      		if(UxState_GetData(&UIFlowWndMovie_Status_GPSCtrl,STATE_CURITEM)!=UIFlowWndMovie_Status_GPS_ICON_GPS_NS)
		      		{
...
INT32 UIFlowWndMovie_OnACPlug(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    ...
    {
#if(_MODEL_DSC_==_MODEL_DUAL_89M3_)		    
        if(bFirst==TRUE)
        {
        	bFirst=FALSE;
        }
...
INT32 UIFlowWndMovie_OnCustom1(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
	...
	||(_MODEL_DSC_ == _MODEL_DUAL_SAIBOSHI_)||(_MODEL_DSC_ == _MODEL_DUAL_GEMEIDI_))
    switch (uiKeyAct)
    { 
      case NVTEVT_KEY_PRESS:
	      if((KeyScan_GetPlugDev() != 1)&&(KeyScan_GetPlugDev() != 2))
...
INT32 UIFlowWndMovie_ReverseGear(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
	...
	g_uiMaskKeyPress = MOVIE_KEY_PRESS_MASK;
	g_uiMaskKeyRelease = MOVIE_KEY_RELEASE_MASK;
	 SxTimer_SetFuncActive(SX_TIMER_DET_AUTOLCDOFF_ID,TRUE);		 
