...
//#NT#2010/07/27#Jeah Yen - begin
    ...
    //load date-time
    cUIDateTime = HwClock_GetCurrentTime();
...
//#NT#2010/07/27#Jeah Yen - end
{
    ...
    HwClock_SetCurrentTime(cUIDateTime);
...
char *DateTime_MakeYMD(void)
{
    ...
    {
        sprintf((char *)gUIDateTime_StrBuf,"%04ld/%02ld/%02ld",
            gUIDateYear,gUIDateMonth,gUIDateDay);
    }
    else if(gUIDateSetFmt == DATEFMT_MDY) //MDY
    {
        sprintf((char *)gUIDateTime_StrBuf,"%02ld/%02ld/%04ld",
...
char *DateTime_MakeYMDHM(void)
{
    ...
    {
        if(gUITimeHour < 12) ti = 1;
        if(gUITimeHour >= 12) ti = 2;
        hr = hr%12;
        if(hr == 0)hr = 12;
    }
    ...
    if(gUIDateSetFmt == DATEFMT_YMD) //YMD
    {
        sprintf((char *)gUIDateTime_StrBuf,"%04ld/%02ld/%02ld %02ld:%02ld%s",
            gUIDateYear,gUIDateMonth,gUIDateDay,hr,gUITimeMinute,Time12HRNameTable[ti]);
    }
    else if(gUIDateSetFmt == DATEFMT_MDY) //MDY
    {
        sprintf((char *)gUIDateTime_StrBuf,"%02ld/%02ld/%04ld %02ld:%02ld%s",
...
char *DateTime_MakeYMDHM_Edit(void)
{
    ...
    sprintf((char *)gUIDateTime_StrBuf+0,"%04ld",gUIDateYear);
    sprintf((char *)gUIDateTime_StrBuf+5,"%02ld",gUIDateMonth);
    sprintf((char *)gUIDateTime_StrBuf+8,"%02ld",gUIDateDay);
    sprintf((char *)gUIDateTime_StrBuf+11,"%02ld",hr);
    sprintf((char *)gUIDateTime_StrBuf+14,"%02ld",gUITimeMinute);
...
char *DateTime_MakeYMDHMS(void)
{
    ...
    if(gUIDateSetFmt == DATEFMT_YMD) //YMD
    {
        sprintf((char *)gUIDateTime_StrBuf,"%04ld/%02ld/%02ld %02ld:%02ld:%02ld%s",
            gUIDateYear,gUIDateMonth,gUIDateDay,hr,gUITimeMinute,gUITimeSecond,Time12HRNameTable[ti]);
    }
    else if(gUIDateSetFmt == DATEFMT_MDY) //MDY
    {
        sprintf((char *)gUIDateTime_StrBuf,"%02ld/%02ld/%04ld %02ld:%02ld:%02ld%s",
...
char *User_MakeYMDHM(UINT32 Year, UINT32 Month, UINT32 Day, UINT32 Hour, UINT32 Minute)
{
    ...
    if(gUIDateSetFmt == DATEFMT_YMD) //YMD
    {
        sprintf((char *)gUIDateTime_StrBuf,"%02ld/%02ld/%02ld %02ld:%02ld%s",
...
char *User_MakeYMDHM_short(UINT32 Year, UINT32 Month, UINT32 Day, UINT32 Hour, UINT32 Minute)
{
    ...
    if(gUIDateSetFmt == DATEFMT_YMD) //YMD
    {
        sprintf((char *)gUIDateTime_StrBuf,"%02ld/%02ld/%02ld %02ld:%02ld",
...
void _TimeRecord_Load(INT32 iTid)
{
    ...
    //load date-time
    ti->Curr_DateTime = HwClock_GetCurrentTime();
}
...
void _TimeRecord_Save(INT32 iTid)
{
    TIME_RECORD* ti = &(gTimeRecord[iTid]);
    //save date-time
    HwClock_SetCurrentTime(ti->Curr_DateTime);
}
...
{
    TIME_RECORD* ti = &(gTimeRecord[iTid]);
    struct tm cUIDateTime = HwClock_GetCurrentTime();
