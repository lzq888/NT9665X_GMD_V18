...
#if(_MODEL_DSC_ == _MODEL_DUAL_GEMEIDI_)	  //JACKY LAN      
extern UINT32 uiUSBOnOff;
#endif
...
#if((_MODEL_DSC_==_MODEL_KERNAL_BOARD_880_)|| (_MODEL_DSC_ == _MODEL_DUAL_GEMEIDI_))
...
#if(ACC_DET_FUNC==ENABLE)	     
SX_TIMER_ITEM(System_DetACC, System_DetACC, 5, FALSE)
#endif
#if (_SENSORLIB2_ != _SENSORLIB2_DUMMY_)
...
void System_DetACC()
{
...
#if(_MODEL_DSC_ == _MODEL_DUAL_GEMEIDI_)	  //JACKY LAN      
   else if(((GxSystem_GetPowerOnSource() == MANUALKEY_SOURCE_POWERON)||(GxSystem_GetPowerOnSource() == USB_SOURCE_POWERON))&&(uiUSBOnOff == ACC_OFF)&&(bFirstPowerOn==TRUE))
   {
		uiAccStatus=ACC_OFF;
		uiAccStatusLast=ACC_ON;
		bFirstPowerOn=FALSE;		
   }   	
#else
   ...
   if(bAccCountFlag==TRUE)
   {
...
#if(_MODEL_DSC_ == _MODEL_DUAL_GEMEIDI_)  		
		if (uiKeyScanDelayoffCnt >=(GxCustom_GetControl(GXCUSTOM_CTRL_DELAYOFF_TIME)*10))
#else
...
#endif
		{
			...
			bAccCountFlag=FALSE;
			uiKeyScanDelayoffCnt=0;
                      debug_msg("System_DetACC Power off :%d,%d,bAccCountFlag:%d\r\n",uiAccStatus,uiAccStatusLast,bAccCountFlag);			
			Ux_PostEvent(NVTEVT_SYSTEM_SHUTDOWN, 1, 0);	            
		}			      
   	}   
	uiAccStatusLast=uiAccStatus;  	
